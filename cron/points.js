      ...
      lineUp.forEach((player) => {
        ...
        playerBowlingStats.forEach(async (bowler) => {
          try {
            ...
            } else if (bowler.economyRate > 3.5 && bowler.economyRate <= 4.49) {
              totalPoints += eBetween3_5And4_5;
              points.economyPoints += eBetween3_5And4_5;
            } else if (bowler.economyRate > 2.5 && bowler.economyRate <= 3.49) {
      ...
      lineUp.forEach((player) => {
        ...
        playerBowlingStats.forEach(async (bowler) => {
          try {
            ...
            } else if (bowler.economyRate > 15.01 && bowler.economyRate <= 16) {
...
const fetchData = async () => {
  try {
    ...
    const fetchMatches = await database(
      "SELECT DISTINCT scorcard_details.matchId AS matchId, fullmatchdetails.matchTyprString AS matchType FROM scorcard_details JOIN fullmatchdetails ON fullmatchdetails.matchId = scorcard_details.matchId WHERE fullmatchdetails.matchRadarId = 32483503;",
      [],
      connection
    );
    let a = 1;
    const pr = async (match) => {
      try {
        ...
        if (batting?.length > 0 && bowling.length > 0 && lineUp.length > 0) {
          if (a === 1) {
            calculatePointsOfMatch(lineUp, batting, bowling, match, connection);
          }
          a++;
        }
      } catch (error) {
