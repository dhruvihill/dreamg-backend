...
router.post("/register", async (req, res) => {
  ...
  try {
    const result = await fetchData("INSERT INTO all_users SET ?", { phoneNumber });
    const jwtData = { user: { userId: result.userId } };
    const token = await jwt.sign(jwtData, process.env.SECRET_KEY);
    res.status(200).json({
      status: true,
      message: "success",
      data: {
...
router.post("/login", async (req, res) => {
  ...
  try {
    const userDetails = await fetchData("SELECT userId,phoneNumber,firstName,lastName FROM all_users where phoneNumber = ?", [phoneNumber]);
...
router.post("/getuserprofile", verifyUser, async (req, res) => {
  ...
  try {
    const points = await fetchData(pointsQuery, [userId, userId, userId, userId, userId,userId]);
    if (points.length > 0) {
      // changing response null to 0
      points[0].mega_contest_totalPoints = points[0].mega_contest_totalPoints | 0;
      points[0].head_to_head_totalPoints = points[0].head_to_head_totalPoints | 0;
...
router.post("/updateuserprofile", verifyUser, async (req, res) => {
  ...
  const updateUserQuery = `UPDATE all_users SET ${keys.join(",")} WHERE userId = ?`;
  ...
  try {
    const updateUserResponse = await fetchData(updateUserQuery, [...values, body.userId]);
    delete body.userId;
